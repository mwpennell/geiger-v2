...
.fix.rjmcmc.matrix=function(mat, colnames){
	...
	mm[,match(colnames(mat), colnames)]=mat
...
#author: JM EASTMAN 2010
summarize.rjmcmc <- function(x, phy=NULL, burnin = NULL, thin = NULL){
	dirs=x
	
	if(length(dirs)==1) {
		return(.subset.auteurRAW(get(load(paste(x, dir(x, pattern="samples.rda"), sep="/"))), phy=phy, burnin=burnin, thin=thin))
	}
	
	# collect individual runs: construct matrices for all unique edges (only necessary if multiple trees run, differing in topology)
	if(.check.multicore()) FUN=mclapply else FUN=lapply
	samples<-FUN(dirs, function(x) {y=.subset.auteurRAW(get(load(paste(x, dir(x, pattern="samples.rda"), sep="/"))), phy=NULL, burnin=burnin, thin=thin); y})
	
	## ENSURE COMPARABILITY of RUNS
	# resolve parameters
	nn=unique(sapply(samples, function(x) {
					 z=sapply(x, function(y) if(!is.null(attributes(y)$parm)) return(TRUE) else return(FALSE))
					 return(names(x)[z])
					 }),MARGIN=2)
	if(ncol(nn)>1) stop("'dirs' do not appear to contain comparable rjmcmc samples -- error in parnames")
	
	# resolve dimnames for parameters
	dimtmp=lapply(nn, function(x) {
				  dimnm=lapply(1:length(samples), function(i) colnames(samples[[i]][[x]]))
				  return(unique(unlist(dimnm)))
				  })
	dmxx=unique(dimtmp)
	if(length(dmxx)>1) stop("'dirs' do not appear to contain comparable rjmcmc samples -- error in dimnames")
	dimnm=unlist(dmxx)
	
	# resolve hashtips for 'hasher'
	hashtmp=lapply(nn, function(x) {
				   htp=lapply(1:length(samples), function(i) attr(samples[[i]][[x]], "hashtips"))
				   return(unique(unlist(htp)))
				   })
	htxx=unique(hashtmp)
	if(length(htxx)>1) stop("'dirs' do not appear to contain comparable rjmcmc samples -- error in hashtips")
	hashtips=unlist(htxx)
	hasher=function(phy) hashes.phylo(phy, tips=hashtips)
	
	# construct parameter matrices based on set of unique edges
	for(i in 1:length(samples)){
		samples[[i]][c(nn)]=lapply(samples[[i]][c(nn)], .fix.rjmcmc.matrix, colnames=dimnm)
	}
		
	# convert matrices to mcmc.list
	csamp=suppressWarnings(to.coda(samples))
	
	# collect trees and resolve variable topologies
	trees=lapply(samples, "[[", "phy")
	class(trees)="multiPhylo"
	csamp$trees=trees
	uu=unique(trees)
	if(length(uu)>1){
		if(is.null(phy)) warning("Encountered multiple trees: a summary tree is recommended (supplied via the 'phy' argument)")
		csamp$phy=NULL
	} else {
		if(!is.null(phy)) {
...
.subset.auteurRAW=function(x, burnin=NULL, thin=NULL, phy=NULL, ...){
...
# rates: a matrix of branchwise rates
# log: a log file
	if(!all("rjmcmc"%in%c(class(rates),class(log)))) stop("Supply 'rjmcmc' objects for 'rates' and 'log'")
	if(!all("function"%in%sapply(dprior, class))) stop("Supply 'dprior' as a list of prior density functions")
	drate<-djumpvar<-dprior$rate
	dshift=dprior$shift
	djump=dprior$jump
	droot=dprior$root
	ss=nrow(log)
	md=apply(rates, 1, median)
	if(nrow(rates)!=ss) stop("objects appear inconsistent")
	pr=sapply(1:ss, function(i){
		drt=sum(drate(rates[i,]))
		dj=djump(jj<-log[i,"jumps"])
		if(jj>0) djv=drate(log[i,"jumpvar"]) else djv=0
		dr=droot(log[i,"root"])
		ds=dshift(log[i,"shifts"])
		return(sum(c(ds,dr, djv, dj, drt)))
...
to.auteur=function(obj, phy=NULL, ...){
	...
	if(any(c("burnin","thin")%in%names(zz))){
	   warning("'burnin' and 'thin' have no effect in this context: use summarize.rjmcmc() on the original output directories.")
	}
...
#	if(run=="bm"){
		con=list(
				 ...
				 hashtips=NULL,													# used with runs across multiple trees
...
#	if(run=="bm"){
		...
		attr(tmp,"cache")=NULL
		if(!is.null(con$hashtips)) {
			if(!all(con$hashtips%in%cache$phy$tip.label)) stop("'hashtips' appears inconsistent with 'phy'")
			cache$hashtips=con$hashtips
		} else {
			cache$hashtips=cache$phy$tip.label
		}
		...
		check.mod=function(parm, null=1){
			if(is.numeric(parm)) parm!=null else !parm
		}
		mod=expand.grid(constrainSHIFT=c(TRUE,FALSE),constrainJUMP=c(TRUE,FALSE))
		rownames(mod)=c("jump-relaxedBM","jump-BM","relaxedBM","BM")
		cur.mod=c(shift=check.mod(con$constrainSHIFT, null=0), jump=check.mod(con$constrainJUMP, null=0))
		con$model=rownames(mod)[modzz<-apply(mod, 1, function(x) all(x==cur.mod))]
		...
		if(is.numeric(con$constrainJUMP)){
			if(!.withinrange(con$constrainJUMP, 0, Inf)) stop("'constrainJUMP' must lie between 0 and Inf.") 
		} else {
...
.edgewise.rjmcmc=function(control, cache){
	thin=control$thin
	ngen=control$ngen
	nd=cache$nodes
	rootd=cache$desc$fdesc[[cache$root]]
	...
	vd=function(string, sep="\t", par=c("shifts", "jumps")){
		...
		attr(mm, "thin")=thin
		attr(mm, "ngen")=ngen
		attr(mm, "hashtips")=cache$hashtips
		attr(mm, "hash")=cache$phy$hash
		if(heritable){
			attr(mm, "shifts")=ss
		}
		mm
...
.startingpt.bm <- function(control, cache) { 
	...
	if(jps>0){
		nd=rep(phy$edge[,2], control$jump.lim)
		jmp=nd[sample(1:length(nd), size=jps, replace=FALSE)]
		jmp=jmp[!jmp%in%control$excludeJUMP]
		if(length(jmp)){
...
#author: JM EASTMAN 2010
.summarize.rjmcmc <- function(control) {
...
#		samples$prior=prior
		samples$phy=read.tree(text=write.tree(cache$phy))
		samples$log=logloc
